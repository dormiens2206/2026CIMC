...
{
    if(p == 0) return 0;
    if(p->magic != PARAM_FLASH_MAGIC) return 0;
    if(p->version != PARAM_FLASH_VERSION) return 0;
    if(p->device_id == 0x0000U || p->device_id == 0xFFFFU) return 0;
    if(!Param_IsBaudCodeValid(p->baud_code)) return 0;
    if(p->dac_raw > 4095U) return 0;
	  if(p->report_interval < REPORT_INTERVAL_1S || p->report_interval > REPORT_INTERVAL_5S) return 0;
    if(p->alarm_mode != ALARM_MODE_ACTIVE && p->alarm_mode != ALARM_MODE_PASSIVE) return 0;
    if(p->crc != Param_CalcCRC(p)) return 0;
    return 1;
}
