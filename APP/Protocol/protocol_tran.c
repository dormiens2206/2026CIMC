...
void Protocol_Dispatch(ProtocolFrame *frame)
{
	...
	{
		// 初始固件版本号为 2.0.1.0
		uint8_t version_buf[4] = {0x02, 0x00, 0x01, 0x00};
	...
	case CMD_CH0_Q: // 0x0201: 查询CH0 (电位器)
		// 读取 ADC -> 乘变比 -> Float2BigEnd 转 4 字节大端
	...
	case CMD_CH1_Q: // 0x0202: 查询CH1 (DAC回读)
		// 读取 ADC -> 乘变比 -> Float2BigEnd 转 4 字节大端
	...
	case CMD_CH2_Q: // 0x0221: 查询特定通道 (PT100温度)
		// 读取温度 -> Float2BigEnd 转 4 字节大端
		//  Protocol_SendFrame(device_id, FRAME_TYPE_REPLY, frame->cmd, temp_buf, 4);
		{
			uint8_t temp_buf[4];
			float temperature;
			...
			// temperature = 25.0f;
			temperature = Sample_GetPT100();
			Protocol_Float2BigEnd(&temperature, temp_buf);
	...
	case CMD_THRESHOLD_R: // 0x0400: 批量读取阈值
	{
		...
		Protocol_SendFrame(device_id, FRAME_TYPE_REPLY, frame->cmd, th_buf, 8);
	}
	break;
	...
	{
		/* 赛题批量阈值只考 CH0/CH1；CH2 暂时返回 0 */
		uint8_t buf[4];
		float th2 = 0.0f;
		Protocol_Float2BigEnd(&th2, buf);
		Protocol_SendFrame(device_id, FRAME_TYPE_REPLY, frame->cmd, buf, 4);
	}
	break;
	...
	{
		/* 当前持久化模块只保存 CH0/CH1，CH2 不参与初赛批量阈值评分 */
		if (frame->len != 4)
		{
			Protocol_SendError(device_id);
			break;
