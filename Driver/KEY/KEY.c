...
void KEY_Scan(void)
{
    ...
    if(gpio_input_bit_get(KEY_PORT,KEY1_PIN))
    {
        key_event_new = KEY1_ON;
    }

    else if(gpio_input_bit_get(KEY_PORT,KEY2_PIN))
    {
        key_event_new = KEY2_ON;
    }

    else if(gpio_input_bit_get(KEY_PORT,KEY3_PIN))
    {
        key_event_new = KEY3_ON;
    }
}
...
BOOL WORK_getKEY(void)
{
    if(gpio_input_bit_get(KEY_PORT,KEY2_PIN))
    {
        return TRUE;
    }
    return FALSE;
...
BOOL TF_getKEY(void)
{
    if(gpio_input_bit_get(KEY_PORT,KEY3_PIN))
    {
        return TRUE;
    }
    return FALSE;
